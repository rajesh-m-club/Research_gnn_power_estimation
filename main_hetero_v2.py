...
def main():
    ...
    trainer = HeteroTrainer(
        ...
        train_loader,
        test_loader,
        lr=1e-3   #  slightly lower than before (important for stability)
    )

    # -----------------------------
    # Train
    # -----------------------------
    trainer.train(epochs=400)   # start with 400 first (not 2000)

    torch.save(model.state_dict(), "toggle_hetero_model_v2.pt")
    print("Model saved")
    ...
    with torch.no_grad():
        ...
        internal_mask = train_mask   # ✔ FIX: directly use train_mask
        ...
        # Plot
        # -----------------------------
        plot_file = "predicted_vs_true_hetero_v2.png"

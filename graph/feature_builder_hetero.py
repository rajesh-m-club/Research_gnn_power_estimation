...
class FeatureBuilderHetero:
    ...
    def build(self):
        ...
        # ========================= PRIMARY =========================
        num_pi = len(self.data.node_map["primary_in"])
        num_po = len(self.data.node_map["primary_out"])

        self.data["primary_in"].x = torch.zeros((num_pi, 1))
        self.data["primary_out"].x = torch.zeros((num_po, 1))

        # IMPORTANT: REMOVE NON-BATCHABLE ATTRIBUTES
